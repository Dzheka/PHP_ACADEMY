-- Создать БД my_counties
CREATE DATABASE `alif_homework_4`;
...
SELECT * FROM (SELECT `cities`.`name` FROM `cities` ORDER BY `cities`.`population` DESC LIMIT 5) AS ORDER BY `area`;

-- Вывести 5 самых многонаселенных стран с вычисленным средним значение (человека на квадратный метр)
SELECT `countries`.`name` AS `country`, SUM(`cities`.`population`) / SUM(`cities`.`area`) AS `density` FROM `countries` INNER JOIN `cities` ON `countries`.`id` = `cities`.`country_id` GROUP BY `countries`.`id` ORDER BY `density` DESC LIMIT 5;

-- Вывести 5 самых многонаселенных стран с вычисленным средним значение (человека на квадратный метр) (детальный)
SELECT `countries`.`name` AS `country`, SUM(`cities`.`population`) AS `all_population`, SUM(`cities`.`area`) AS `all_area`, SUM(`cities`.`population`) / SUM(`cities`.`area`) AS `density` FROM `countries` INNER JOIN `cities` ON `countries`.`id` = `cities`.`country_id` GROUP BY `countries`.`id` ORDER BY `density` DESC LIMIT 5;
...
UPDATE `cities` SET `cities`.`density` = (cities.population / cities.area) WHERE 1;
...
-- удалить города где is_active равен 0
DELETE FROM `cities` WHERE `cities`.`is_active` = FALSE;

-- Удалить 2 страны с наименьшим количеством городов
SELECT `countries`.`name`, COUNT(`cities`.`name`) AS `citiesCount` FROM `countries` INNER JOIN `cities` ON `countries`.`id`=`cities`.`country_id` GROUP BY `countries`.`id` ORDER BY `citiesCount` ASC LIMIT 2;
